...
* getFront() : 데크의 첫번째 아이템 가져온다. 데크가 비어 있다면 -1을 리턴한다.
...
class MyCircularDeque():
    '''
    Deque : 양쪽 끝을 모두 추출 가능한, Queue를 일반화한 형태의 추상자료형(ADT).
            양쪽에서 삭제/삽입을 모두 처리 가능하며, 스택/큐의 특징을 모두 포함.
    ...
    def insertFront(self, value):
        '''
        앞쪽에 노드를 추가하는 함수
        새로운 노드 삽입 시 최대 길이 도달하면 False를, 이외에는 _add() 함수로
        head 위치에 노드 삽입
        '''
        if self.len == self.k:
            return False
